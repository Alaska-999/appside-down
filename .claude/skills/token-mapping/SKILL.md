...
The equally real opposite failure mode: forcing everything into named tokens even when a value is genuinely mockup-specific and won't recur. Forcing that into a fake token used in exactly one place adds indirection with no reuse benefit. Read `docs/superpowers/mockups/elements-v1.html` (the source of truth) for the value's origin before deciding — if the mockup shows the same number reused across 2+ element types, it's a token; if it's local to one component, a literal is correct and simpler (no code comments in this repo — the mockup itself is the record).
...
- **Typography scale from elements-v1** (roles, not ad-hoc sizes): display 30/800, title 23/800, section 17/700, card 16/700, body 15/400, meta 12.5/400, micro 11/600. Note 12.5 is an official scale step now, not a one-off literal. The legacy `src/constants/typography.ts` `TEXT` scale is being replaced by this — don't add new consumers to `TEXT`.
...
3. **Decide token vs. literal** using the reuse test above. If it's genuinely one-off, leave it as a literal. No code comments (project rule) — traceability lives in the mockup file, not inline.
4. **If it should be a token:**
   - Add the base value to `tokens.color`/`tokens.space`/`tokens.radius` in `tamagui.config.ts`. New COLOR tokens require the user's explicit approval — the elements-v1 palette is closed.
