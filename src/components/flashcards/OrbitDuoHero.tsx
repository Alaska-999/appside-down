import { OrbitBead } from "@/src/components/flashcards/OrbitBead";
import {
  CENTER,
  ORB_SIZE,
  PLANET_R,
...
const THIN_ORBITS: ThinOrbit[] = [
  {
    ...
    ratio: 0.28,
    tilt: 45,
    color: withAlpha(ICON_MINT_LIGHT, 0.75),
    width: 1.4,
    dash: [1.5, 8],
    pulseMs: 3600,
    delayMs: 0,
  },
  {
    ...
    ratio: 0.28,
    tilt: -45,
    color: withAlpha(ICON_LIME_LIGHT, 0.6),
    width: 1.4,
    dash: [1.5, 8],
    pulseMs: 4800,
    delayMs: 1200,
...
function ThinOrbitPath({
  ...
  const mirror = orbit.tilt < 0 ? -1 : 1;
  const bob = useSharedValue(-BOB_PX * mirror);
  const tilt = useSharedValue(-1);

  useEffect(() => {
    if (reducedMotion) return;
    bob.value = withRepeat(
      withTiming(BOB_PX * mirror, {
        duration: BOB_MS,
    ...
    tilt.value = withRepeat(
      withTiming(1, {
        duration: TILT_SWING_MS,
        easing: Easing.inOut(Easing.ease),
      }),
      -1,
      true,
    );
  }, [reducedMotion, orbit, mirror, pulse, phase, bob, tilt]);
  ...
  const opacity = useDerivedValue(() =>
    side === "front" ? pulse.value : pulse.value * 0.72,
  );
  const sway = useDerivedValue(() => [{ translateY: bob.value }]);
  const baseRad = toRad(orbit.tilt);
  const swingRad = toRad(TILT_SWING_DEG * mirror);
...
function FadeSegment({
  ...
  return (
    <ArcStroke
      ...
      opacity={layer === "glow" ? step.glow : step.body}
    />
  );
}
...
interface OrbitDuoHeroProps {
  progress: SharedValue<number>;
  fraction: number;
  hot?: boolean;
}

export function OrbitDuoHero({
  progress,
  fraction,
  hot = false,
}: OrbitDuoHeroProps) {
    ...
    <View style={{ width: ORB_SIZE, height: ORB_SIZE }}>
      <Canvas style={CANVAS_STYLE} pointerEvents="none">
        <Group transform={[{ translateX: PAD }, { translateY: PAD }]}>
          ...
          <ThinOrbits side="back" reducedMotion={reducedMotion} />
          <PlanetGlow
            cx={CENTER}
            cy={CENTER}
            state={state}
          ...
          >
            ...
            <ProgressArc path={backArc} from={0.5} to={1} end={backLateEnd} />
            <ProgressArc path={backArc} from={0} to={0.5} end={backEarlyEnd} />
      ...
      <Canvas style={CANVAS_STYLE} pointerEvents="none">
        <Group transform={[{ translateX: PAD }, { translateY: PAD }]}>
          ...
          >
            ...
            <ProgressArc path={frontArc} from={0} to={1} end={frontEnd} />
            {hot && <ClosureFlash path={frontArc} closure={closure} />}
          </Group>
          <ThinOrbits side="front" reducedMotion={reducedMotion} />
        </Group>
      </Canvas>
